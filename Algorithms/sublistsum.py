...
# answer(l, t) which verifies if there is at least one consecutive sequence of positive integers
...
# there is no such sequence (to throw off Lambda's spies, not all number broadcasts will contain
# a coded message).

# For example, given the broadcast list l as [4, 3, 5, 7, 8] and the key t as 12, the function
# answer(l, t) would return the list [0, 2] because the list l contains the sub-list [4, 3, 5]
# starting at index 0 and ending at index 2, for which 4 + 3 + 5 = 12, even though there is a
# shorter sequence that happens later in the list (5 + 7). On the other hand, given the list l as
# [1, 2, 3, 4] and the key t as 15, the function answer(l, t) would return [-1, -1] because there is
# no sub-list of list l that can be summed up to the given target value t = 15.
...
# - For the list returned by answer(l, t), the start index must be equal or smaller than the end
# index.
...
# hidden in the first sublist that sums up to the key, so answer(l, t) should only return that
# sublist.
...
# sum[i][j]: the sum of the integer list that starts from x and ends at y inclusive
# if sum[i][j] >t, forget about s[i][k] for all k>=j
# if sum[i][j] <t, continue
# if sum[i][j] =t, we have arrived at a possible solution
# to find the smallest possible sum, start with smallest i, exhaust all possibility of j before moving on to i+1
# sum[i][j+1] = sum[i][j]+l[j+1]
...
def answer(l, t):
    ...
    for i in range(n):
        for j in range(i, n-1):
            print(i, j)
            # exceed t, forget about this i
            if sums[i][j] > t:
                break
            sums[i][j+1] = sums[i][j]+l[j+1]
            if sums[i][j+1] == t:
                # found it
                return [i, j+1]
    return [-1, -1]
...
ans = answer([4, 3, 10, 2, 8], 12)
print(ans)
ans = answer([1, 2, 3, 4], 15)
print(ans)
